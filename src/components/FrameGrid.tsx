  ...
  return (
    <div className="flex w-[70%] flex-col items-center justify-center gap-8 px-4 pb-12 pt-10 sm:gap-10 sm:pb-16">
      {GALLERY_FRAMES.map((frame, index) => (
        <Frame key={frame.src} frame={frame} index={index} />
      ))}
    </div>
  )
}
...
const Frame = ({ frame, index }: { frame: GalleryFrame; index: number }) => {
  ...
  return (
    <div
      className={`relative flex items-end gap-8 pb-6 sm:gap-12 sm:pb-8 md:gap-16 ${
        side === 'left' ? 'self-start flex-row' : side === 'center' ? 'self-center flex-row' : 'self-end flex-row-reverse'
      }`}
